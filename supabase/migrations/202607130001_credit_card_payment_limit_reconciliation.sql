...
-- The baseline exposed the monthly/yearly keys as text. These reconciled
-- views intentionally use dates, which PostgreSQL cannot apply with
-- CREATE OR REPLACE VIEW. Rebuild the dependent dashboard in dependency
-- order so a fresh canonical migration chain is executable.
drop view if exists public.v_dashboard_summary;
drop view if exists public.v_yearly_income_expense;
drop view if exists public.v_monthly_income_expense;
