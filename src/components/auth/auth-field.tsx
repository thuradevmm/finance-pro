...
export function AuthField({ autoComplete, error, icon, label, name, onChange, placeholder, type = "text", value }: AuthFieldProps) {
  ...
  return (
    <div>
      <label className="mb-2 block text-xs font-bold uppercase text-[#45464d]" htmlFor={name}>{label}</label>
      ...
      {error ? <p className="mt-1.5 text-xs font-medium text-[#ba1a1a]" id={`${name}-error`}>{error}</p> : null}
    </div>
  );
}
