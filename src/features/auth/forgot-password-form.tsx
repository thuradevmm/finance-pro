...
export function ForgotPasswordForm() {
  ...
  if (submittedEmail) {
    return (
      <div>
        ...
        <p className="mt-2 text-sm leading-6 text-[#5f6168]">A password reset link was sent to <strong className="text-[#0b1c30]">{submittedEmail}</strong>.</p>
  ...
  return (
    <form className="space-y-5" noValidate onSubmit={handleSubmit}>
      ...
      <button className="inline-flex h-12 w-full items-center justify-center rounded-md bg-[#0b1c30] px-5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#1f2937] focus:outline-none focus:ring-2 focus:ring-[#2170e4] focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60" disabled={isSubmitting} type="submit">{isSubmitting ? "Sending…" : "Send Reset Link"}</button>
