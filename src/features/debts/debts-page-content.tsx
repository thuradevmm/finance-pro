...
import { cancelDebt, deleteDebt, undoDebtCancellation } from "@/app/debts/actions";
...
function DebtsTable({
  ...
  onUndoCancellation: (id: string) => Promise<boolean>;
    ...
    <section className="min-w-0 max-w-full overflow-hidden rounded-lg border border-[#c6c6cd]/70 bg-white shadow-sm">
      <div className="flex min-w-0 flex-col items-stretch gap-3 border-b border-[#c6c6cd]/60 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs font-semibold text-[#45464d]">{showActiveOnly ? "Showing active and overdue borrowing and lending records" : "Showing paid and canceled records too"}</p>
        </div>
      </div>
            ...
            {sortedDebts.map((debt) => (
              <tr className="transition hover:bg-[#f8f9ff]" key={debt.id}>
                ...
                <td className="px-4 py-4 text-center">
                  <span className={`inline-flex rounded px-2 py-1 text-xs font-bold ${statusStyles[debt.isCanceled ? "Canceled" : debt.status]}`}>{debt.isCanceled ? "Canceled" : debt.status}</span>
                </td>
                <td className="px-4 py-4">
                  <div className="flex justify-end gap-1">
                    {debt.isCreditCardDebt && !debt.usesManualCreditCardTerms ? (
                      ...
                      <RecordLifecycleActions
                        deactivateDescription={`Cancel ${debt.name} and remove its remaining ${debt.nature === "Lending" ? "receivable" : "obligation"} from financial summaries and dashboard analysis. Posted account transactions remain unchanged for audit history.`}
                        deactivateLabel="Cancel record"
                        deactivatePendingLabel="Canceling…"
                        deactivateTitle="Cancel Borrowing & Lending record"
                        ...
                        isInactive={debt.isCanceled}
                        itemId={debt.id}
                        itemLabel={debt.name}
                        onDeactivate={onCancel}
                        onDelete={onDelete}
                        onRestore={onUndoCancellation}
                        onView={() => onView(debt)}
                        restoreDescription={`Undo the cancellation of ${debt.name}. Its remaining ${debt.nature === "Lending" ? "receivable" : "obligation"} will return to financial summaries, dashboard analysis, schedules, and related activity.`}
                        restoreLabel="Undo cancellation"
                        restorePendingLabel="Undoing…"
                        restoreTitle="Undo Borrowing & Lending cancellation"
                        showDelete={!debt.hasFinancialHistory}
                        showEdit={!debt.isCanceled}
                      />
                    )}
                  </div>
                </td>
          ...
          {sortedDebts.map((debt) => (
            <article className="min-w-0 rounded-lg border border-[#c6c6cd]/60 bg-white p-4 shadow-sm" key={`mobile-${debt.id}`}>
              <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                ...
                <span className={`w-fit shrink-0 rounded px-2 py-1 text-xs font-bold ${statusStyles[debt.isCanceled ? "Canceled" : debt.status]}`}>{debt.isCanceled ? "Canceled" : debt.status}</span>
              </div>
...
export function DebtsPageContent({ accounts, debts, payments }: { accounts: AccountRecord[]; debts: DebtRecordWithValues[]; payments: UpcomingDebtPayment[] }) {
  ...
  const filteredDebts = useMemo(() => {
    ...
    return visibleDebts.filter((debt) => {
      ...
      const statusMatches = !showActiveOnly || (!debt.isCanceled && debt.status !== "Paid");
  ...
  async function handleCancel(debtId: string) {
    setIsPending(true);
    const result = await cancelDebt(debtId);
    setIsPending(false);
    if (result.error) {
      showError(result.error);
      return false;
    }
    setVisibleDebts((items) => items.map((item) => item.id === debtId ? { ...item, isArchived: true, isCanceled: true } : item));
    showSuccess("Record canceled; its remaining financial position was removed while transaction history was preserved.");
    return true;
  }

  async function handleUndoCancellation(debtId: string) {
    setIsPending(true);
    const result = await undoDebtCancellation(debtId);
    setIsPending(false);
    if (result.error) {
      showError(result.error);
      return false;
    }
    setVisibleDebts((items) => items.map((item) => item.id === debtId ? { ...item, isArchived: false, isCanceled: false } : item));
    showSuccess("Cancellation undone; the remaining financial position and related activity were restored.");
    return true;
  }
      ...
      >
        {viewedDebt ? <div className="space-y-5">
          <DetailModalSection title="Record information">
            ...
            <DetailModalField label="Status" value={viewedDebt.isCanceled ? "Canceled" : viewedDebt.status} />
