...
function relatedOptions(
  ...
  return [
    ...
    ...savingsGoals.filter((goal) => !goal.isArchived || preserves("savings_goal", goal.id)).map((goal) => ({
    ...
    ...debts.filter((debt) => (!debt.isArchived && debt.status !== "Paid") || preserves("debt", debt.id)).map((debt) => ({
