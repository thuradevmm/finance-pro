...
export default async function EditAccountPage({
  ...
  if (!user) notFound();
  const [account, categories, amountTypeCatalog] = await Promise.all([
    getAccount(supabase, user.id, accountId),
    getCategories(),
    getAccountAmountTypeCatalog(supabase, user.id),
  ]);
  ...
  return (
    ...
    >
      ...
      <AddAccountForm account={account} amountTypeCatalog={amountTypeCatalog} categories={categories} returnTo={returnTo} />
    </AppShell>
  );
}
