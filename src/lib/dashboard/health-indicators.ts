...
export function buildFinancialHealthSignals(input: {
  ...
  for (const transaction of input.transactions) {
    ...
    if (transaction.relatedEntityType === "savings_goal" && transaction.relatedEntityId) {
      netSavings = roundCurrencyValue(netSavings + savingsTransactionDelta({
        ...
        type: transaction.type,
      }, goalAccountById.get(transaction.relatedEntityId) ?? ""));
    }
  }
  ...
  const cashFlow: FinancialHealthSignal = income <= 0 && expenses <= 0
  ...
  const essentialLoad: FinancialHealthSignal = expenses > 0 && essentialExpenses <= 0
  ...
  const savingMomentum: FinancialHealthSignal = savingsRate == null
