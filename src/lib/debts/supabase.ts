...
function mapDebt(row: DebtRow, categories: Map<string, CategoryRecord>, linkedRepaymentsByDebtId: Map<string, number>): DebtRecordWithValues {
  ...
  const type = String(row.type ?? metadata.type ?? "Personal Loan");
  ...
  const totalAmountValue = numericValue(row.total_amount ?? metadata.total_amount);
  const repaidAmountValue = numericValue(row.repaid_amount ?? metadata.repaid_amount) + (linkedRepaymentsByDebtId.get(row.id) ?? 0);
  const monthlyPaymentValue = numericValue(row.monthly_payment ?? metadata.monthly_payment);
  ...
  return {
    ...
    categoryId,
    id: row.id,
    interestRate: `${numericValue(row.interest_rate ?? metadata.interest_rate)}%`,
    interestRateValue: numericValue(row.interest_rate ?? metadata.interest_rate),
    ...
    nextPaymentDateValue: row.next_payment_date ?? "",
    ...
    repaidAmountValue,
    startDate: row.start_date ?? (typeof metadata.start_date === "string" ? metadata.start_date : ""),
